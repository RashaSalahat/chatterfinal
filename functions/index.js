...
const functions = require("firebase-functions"); //to call functions
const admin = require("firebase-admin"); //admin functionalities

admin.initializeApp(); 

//make an  instance that require a key  & your stream secret
...
//Call this if you are authenticated
// regitster the call back on our functions : On http score
...
exports.createStreamUserAndGetToken = functions.https.onCall(async (data, context) => {
  ...
  // if the auth object is null or not
  if (!context.auth) {
...
exports.getStreamUserToken = functions.https.onCall((data, context) => {
  ...
  if (!context.auth) {
    ...
    try {
      ...
      // firebase identifier for a user:    (context.auth.uid);
      //
      return serverClient.createToken(context.auth.uid);
    } catch (err) {
